...
#'
#' # A virtual population of N = 21,483 people, of which 4119 consider their party identification to be "Independent".
# population <- forcats::gss_cat %>%
#   select(partyid)
#
# # Take samples of size n = 50 people without replacement; do this 1000 times
# samples <- population %>%
#   rep_sample_n(size = 50, reps = 1000)
# samples
#
# # Compute p_hats for all 1000 samples = proportion Independent
# p_hats <- samples %>%
#   group_by(replicate) %>%
#   summarize(prop_red = mean(partyid == "Independent"))
# p_hats
#
