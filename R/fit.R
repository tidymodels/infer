...
#' @export fit.infer
#' @export
fit.infer <- function(object, spec = NULL, summarize = summarize_infer, ...) {
  ...
  if (is_generated(object)) {
      ...
      dplyr::mutate(
        model = list(
          do.call(
            fit_infer_model,
            c(
              list(object = data, spec = spec, formula = formula, summarize = summarize),
              dots
            )
          )
        )
    ...
    x <- do.call(
      fit_infer_model,
      c(
        list(object = object, spec = spec, formula = formula, summarize = summarize),
        dots
      )
    )
  }
...
fit_infer_model <- function(object, spec, formula, summarize, ...) {
   if (is.null(spec)) {
      res <- stats::glm(formula = formula, data = object, ...)
   } else {
      res <- fit(spec, formula = formula, data = object, ...)
   }

   summarize(res, object)
}

summarize_infer <- function(x, data) {
   broom::tidy(x) %>%
      dplyr::select(
         term,
         estimate
