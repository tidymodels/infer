...
Given the output of
[`specify()`](https://infer.tidymodels.org/reference/specify.md) and/or
...
function will calculate the supplied `stat` for each `replicate`.
...
- x:
  ...
  The output from
  [`generate()`](https://infer.tidymodels.org/reference/generate.md) for
  computation-based inference or the output from
  [`hypothesize()`](https://infer.tidymodels.org/reference/hypothesize.md)
