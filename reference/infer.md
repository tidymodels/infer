# infer: a grammar for statistical inference

The objective of this package is to perform statistical inference using
a grammar that illustrates the underlying concepts and a format that
coheres with the tidyverse.

## Details

For an overview of how to use the core functionality, see
[`vignette("infer")`](https://infer.tidymodels.org/articles/infer.md)

## See also

Useful links:

- <https://github.com/tidymodels/infer>

- <https://infer.tidymodels.org/>

- Report bugs at <https://github.com/tidymodels/infer/issues>

## Author

**Maintainer**: Simon Couch <simon.couch@posit.co>
([ORCID](https://orcid.org/0000-0001-5676-5107))

Authors:

- Andrew Bray <abray@reed.edu>

- Chester Ismay <chester.ismay@gmail.com>
  ([ORCID](https://orcid.org/0000-0003-2820-2547))

- Evgeni Chasnovski <evgeni.chasnovski@gmail.com>
  ([ORCID](https://orcid.org/0000-0002-1617-4019))

- Ben Baumer <ben.baumer@gmail.com>
  ([ORCID](https://orcid.org/0000-0002-3279-0516))

- Mine Cetinkaya-Rundel <mine@stat.duke.edu>
  ([ORCID](https://orcid.org/0000-0001-6452-2420))

Other contributors:

- Ted Laderas <tedladeras@gmail.com>
  ([ORCID](https://orcid.org/0000-0002-6207-7068)) \[contributor\]

- Nick Solomon <nick.solomon@datacamp.com> \[contributor\]

- Johanna Hardin <Jo.Hardin@pomona.edu> \[contributor\]

- Albert Y. Kim <albert.ys.kim@gmail.com>
  ([ORCID](https://orcid.org/0000-0001-7824-306X)) \[contributor\]

- Neal Fultz <nfultz@gmail.com> \[contributor\]

- Doug Friedman <doug.nhp@gmail.com> \[contributor\]

- Richie Cotton <richie@datacamp.com>
  ([ORCID](https://orcid.org/0000-0003-2504-802X)) \[contributor\]

- Brian Fannin <captain@pirategrunt.com> \[contributor\]
