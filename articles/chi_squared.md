...
null distribution in one of two ways—using randomization or theory-based
methods. The randomization approach approximates the null distribution
by permuting the response and explanatory variables, so that each
person’s educational attainment is matched up with a random income from
the sample in order to break up any association between the two.
...
[`assume()`](https://infer.tidymodels.org/reference/assume.md) verb to
define a theoretical null distribution and then pass it to
[`visualize()`](https://infer.tidymodels.org/reference/visualize.md)
