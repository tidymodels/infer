...
The syntax to do the same thing with the theoretical null distribution looks similar---we just pipe in the theoretical null distribution instead, and drop in `method = "theoretical"` to `visualize()`.
...
We could also visualize both null distributions to get a sense of how the simulation-based and theoretical versions relate. To do so, start off with the simulation-based null distribution, and then drop in `method = "both"` to `visualize()`.
...
# calculate the p value from the simulation-based null 
