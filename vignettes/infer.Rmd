...
{infer} also provides functionality to use theoretical methods for the following test statistics: `"Chisq"`, `"F"`, `"t"`, and `"z"`. 

Generally, to find a null distribution using theory-based methods, use the same code that you would use to find the null distribution using simulation-based methods, but skip the `generate()` step. For example, if we wanted to find a null distribution for the relationship between age (`age`) and party identification (`partyid`) using simulation, we could write:
...
Now, instead of just piping the null distribution into `visualize()`, as we would do if we wanted to visualize the simulation-based null distribution, we also need to provide `method = "theoretical"` to `visualize()`.
...
To get a sense of how the theory-based and simulation-based null distributions relate, as well, we can pipe the simulation-based null distribution into `visualize()` and also specify `method = "both"`
