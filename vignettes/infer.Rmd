...
null_dist %>%
  visualize(obs_stat = point_estimate)
```

The red bar looks like it's pretty far out on the right tail of the null distribution, so observing a mean of `r point_estimate` hours would be pretty unlikely if the mean was actually 40 hours. How unlikely, though?
...
We can also shade the regions of the null distribution that are as (or more) extreme than our observed statistic to get a sense of what `r p_value*100`% of the distribution actually looks like.

```{r shade_p_value, warning = FALSE, message = FALSE}
# get a two-tailed p-value
null_dist %>%
  visualize() +
